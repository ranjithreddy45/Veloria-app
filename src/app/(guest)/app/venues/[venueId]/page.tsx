...
// filled that field. The sticky bar carries the price and the way in to the
// reserve flow, and never claims a date is free or held.
// ============================================================
...
export default async function VenueDetailPage({ params }: { params: Promise<{ venueId: string }> }) {
  ...
  return (
    <div className="vg-rise">
      ...
      <div className="relative z-[1] -mt-6 flex flex-col gap-[22px] rounded-t-3xl bg-[#f3f0ec] px-5 pt-[22px]">
        {/* 2 — what this hall is */}
        <section>
          ...
          {cover.isStock && (
            <p className="mt-3 text-meta leading-[1.5] text-[#8a8a8e]">
          ...
          {venue.description && <p className="mt-3 text-body leading-[1.6] text-[#3a3a3c]">{venue.description}</p>}
        </section>
        ...
        {/* 4 — the calendar, and 5 — the sticky bar it feeds */}
        <HallBooking venueId={venue.id} venueName={venue.name} priceAmount={price.amount} perGuest={price.perGuest} />

        {/* 6 — the practical facts, each only once the team has filled it in */}
            ...
            <div className="mt-2.5 flex flex-col gap-2">
              {video && video.kind === "link" && (
                ...
                >
                  ...
                  <ExternalLink className="size-3.5 text-[#8a8a8e]" aria-hidden />
                </a>
              )}
              {tourHref && (
                <a
