  ...
  return (
    <div className="space-y-4">
      ...
      <div
        ...
        className="mx-auto w-full max-w-[820px] rounded-lg border border-border/70 bg-white p-10 text-[13.5px] leading-relaxed text-[#2D1B3D] shadow-card sm:p-14"
      ...
      >
        ...
        >
          <div>
            <div className="text-[22px] font-extrabold tracking-tight" style={{ color: PLUM }}>
              Veloria Grand
            </div>
            <div
            ...
            <div className="mt-1 text-[10.5px] text-[#6b5b73]">{COMPANY_LEGAL_LINE}</div>
            <div className="text-[10.5px] text-[#6b5b73]">{COMPANY_ADDRESS}</div>
            {COMPANY_GSTIN && (
              <div className="text-[10.5px] text-[#6b5b73]">GSTIN: {COMPANY_GSTIN}</div>
            )}
          </div>
          <div className="text-right">
            <div className="text-[15px] font-bold" style={{ color: PLUM }}>
              Letter of Offer
            </div>
            <div className="text-[11px] text-[#6b5b73]">Private &amp; Confidential</div>
          </div>
        </div>

        {/* Offer summary chips */}
        <div className="my-6 grid grid-cols-2 gap-x-6 gap-y-2 text-[12px] sm:grid-cols-3">
        ...
        {/* Footer */}
        <div
          className="mt-10 flex items-center justify-between pt-4 text-[10.5px] text-[#6b5b73]"
...
function Field({ label, value }: { label: string; value: string }) {
  return (
    <div className="flex flex-col gap-0.5 border-b border-dashed border-[#e6dccb] pb-1.5">
      <span className="text-[10px] font-semibold uppercase tracking-[0.06em] text-[#9a8ca3]">
        {label}
      </span>
