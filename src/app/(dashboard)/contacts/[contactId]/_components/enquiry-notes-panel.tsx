...
// ============================================================
// Enquiry notes + call log — the same multi-note thread leads have, scoped to
// a Contact. Reads/writes go through the shared crm-note actions, so an
// enquiry note is the same record type as a lead note.
// ============================================================
...
import {
  listCrmNotes, addCrmNote, editCrmNote, deleteCrmNote, type CrmNoteDTO,
} from "@/actions/crm-note.actions";
...
/** Editable notes + call log for an enquiry. Multiple notes; author edits/deletes own. */
export function EnquiryNotesPanel({ contactId }: { contactId: string }) {
  ...
  const load = useCallback(async () => {
    const res = await listCrmNotes({ contactId });
  ...
  async function add() {
    ...
    try {
      const res = await addCrmNote({
        contactId, kind: mode, body,
        callOutcome: mode === "CALL" ? callOutcome : null,
      });
    ...
    <Card>
      <CardHeader className="pb-3">
        <CardTitle className="flex items-center gap-2 text-base">
          <StickyNote className="size-4 text-violet-600" /> Notes &amp; calls
        </CardTitle>
      </CardHeader>
      <CardContent className="space-y-3">
        ...
        <div className="space-y-2 rounded-lg border bg-muted/30 p-2.5">
          <div className="flex items-center gap-2">
            ...
            <Button size="sm" variant={mode === "CALL" ? "default" : "outline"} className="h-7 gap-1.5" onClick={() => setMode("CALL")}>
          ...
          <Textarea
            ...
            rows={2}
            placeholder={mode === "CALL" ? "What was discussed on the call?" : "Add a note…"}
            className="text-sm"
          />
          <Button size="sm" className="gap-1.5" onClick={add} disabled={busy || !draft.trim()}>
            {busy ? <Loader2 className="size-3.5 animate-spin" /> : <Plus className="size-3.5" />}
            {mode === "CALL" ? "Log call" : "Add note"}
          </Button>
        </div>
                  ...
                  {n.canEdit && editId !== n.id && (
                    <span className="ml-auto flex gap-1">
                      <button
                        ...
                        onClick={() => { setEditId(n.id); setEditBody(n.body); }}
                      >
                ...
                {editId === n.id ? (
                  <div className="space-y-1.5">
                    ...
                    <div className="flex gap-1.5">
                      <Button size="sm" className="h-7 gap-1" onClick={() => saveEdit(n.id)} disabled={busy}>
                        <Check className="size-3.5" /> Save
                      </Button>
                      <Button size="sm" variant="ghost" className="h-7 gap-1" onClick={() => setEditId(null)}>
