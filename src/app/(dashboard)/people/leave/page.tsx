...
export default async function LeavePage() {
  ...
  return (
    <div className="space-y-6">
      ...
      {needsSetup ? (
        canAdmin ? <LeaveSetup /> : (
          <div className="rounded-[22px] border border-dashed bg-card/40 p-10 text-center text-body text-muted-foreground">
            Leave hasn’t been set up yet. Ask an HR Manager to configure leave types.
          </div>
        )
      ) : !dashboard ? (
        <div className="rounded-[22px] border border-dashed bg-card/40 p-10 text-center text-body text-muted-foreground">Please sign in.</div>
