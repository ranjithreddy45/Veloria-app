  ...
  return (
    <Dialog open={open} onOpenChange={handleOpenChange}>
      ...
      <DialogContent className="max-h-[90vh] max-w-xl overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="text-lede font-semibold">
        ...
        <form onSubmit={handleSubmit} className="mt-2 space-y-5">
          ...
          <div className="space-y-1.5">
            <Label htmlFor="vf-name" className="text-body font-medium">
            ...
            <Input
              ...
              className={cn("h-9 text-body", fieldErrors.name && "border-destructive")}
              autoFocus
            />
            {fieldErrors.name && (
          ...
          <div className="space-y-2">
            ...
            {fieldErrors.categories && (
              <p className="text-detail text-destructive">{fieldErrors.categories}</p>
            )}
          </div>

          {/* ── Vendor type ── */}
          <div className="space-y-1.5">
            <Label htmlFor="vf-type" className="text-body font-medium">
              Vendor type
            </Label>
            <Select value={vendorType} onValueChange={setVendorType}>
              <SelectTrigger id="vf-type" className="h-9 text-body">
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
                {VENDOR_TYPE_OPTIONS.map((t) => (
                  <SelectItem key={t.key} value={t.key} className="text-body">
                    {t.label}
                  </SelectItem>
                ))}
              </SelectContent>
            ...
            {fieldErrors.vendorType && (
              <p className="text-detail text-destructive">{fieldErrors.vendorType}</p>
            )}
          </div>

          {/* ── Venue scope ── */}
          <div className="space-y-2">
            <Label className="text-body font-medium">Assigned venues</Label>
            <div className="flex flex-wrap gap-2">
              <button
                type="button"
            ...
            {!allVenues && (
              <div className="max-h-40 space-y-1.5 overflow-y-auto rounded-lg border border-border/70 bg-muted/20 p-2.5">
                {venues.length === 0 ? (
                  <p className="text-detail text-muted-foreground/70">No active venues found.</p>
                ) : (
                  venues.map((v) => (
                    <label
                      key={v.id}
                      className="flex cursor-pointer items-center gap-2 text-detail"
                    >
                      <Checkbox
          ...
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
            <div className="space-y-1.5">
              <Label htmlFor="vf-contact" className="text-body font-medium">
                Primary contact
              </Label>
              <Input
                id="vf-contact"
            ...
            <div className="space-y-1.5">
              <Label htmlFor="vf-phone" className="text-body font-medium">
                Phone
              </Label>
              <Input
                id="vf-phone"
            ...
            <div className="space-y-1.5">
              <Label htmlFor="vf-email" className="text-body font-medium">
                Email
              </Label>
              <Input
                id="vf-email"
                ...
                className={cn("h-9 text-body", fieldErrors.email && "border-destructive")}
              />
              {fieldErrors.email && (
                <p className="text-detail text-destructive">{fieldErrors.email}</p>
              )}
            </div>

            <div className="space-y-1.5">
              <Label htmlFor="vf-city" className="text-body font-medium">
                City
              </Label>
              <Input
                id="vf-city"
          ...
          {/* ── Empanelment status ── */}
          <div className="space-y-1.5">
            <Label htmlFor="vf-status" className="text-body font-medium">
              Empanelment status
            </Label>
            <Select value={empanelmentStatus} onValueChange={setEmpanelmentStatus}>
              <SelectTrigger id="vf-status" className="h-9 text-body">
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
                {VENDOR_EMPANELMENT_STATUSES.map((s) => (
                  <SelectItem key={s} value={s} className="text-body">
            ...
            {fieldErrors.empanelmentStatus && (
              <p className="text-detail text-destructive">{fieldErrors.empanelmentStatus}</p>
            )}
          </div>
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              <Label className="text-body font-medium">Key personnel</Label>
              <Button
                type="button"
                variant="outline"
                size="sm"
                className="h-7 gap-1 text-detail"
            ...
            {keyPersonnel.length === 0 && (
              <p className="text-detail text-muted-foreground/70">No key personnel added.</p>
            )}
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              <Label className="text-body font-medium">Licences &amp; certifications</Label>
              <Button
                type="button"
                variant="outline"
                size="sm"
            ...
            {licences.length === 0 && (
              <p className="text-detail text-muted-foreground/70">No licences added.</p>
            )}
                ...
                <div key={i} className="flex items-start gap-2">
                  <div className="grid flex-1 grid-cols-3 gap-2">
                    ...
                    <Input
                      ...
                      placeholder="Number"
                      className="h-8 text-detail tabular-nums"
                    />
                    <Input
                      type="date"
                      value={lic.expiry}
          ...
          {/* ── Notes ── */}
          <div className="space-y-1.5">
            <Label htmlFor="vf-notes" className="text-body font-medium">
              Notes
            </Label>
            <Textarea
              id="vf-notes"
          ...
          <DialogFooter className="gap-2 pt-2">
            ...
            <Button
              type="submit"
              className="min-w-[80px] text-body"
