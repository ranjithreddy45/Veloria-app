    ...
    <Table className="[&_td]:px-2.5 sm:[&_td]:px-4">
      <TableBody>
        {rows.length === 0 ? (
          ...
          <TableRow key={l.code}>
            <TableCell className="whitespace-normal text-body"><span className="numeric text-muted-foreground">{l.code}</span> {l.name}</TableCell>
            <TableCell className="numeric whitespace-nowrap text-right text-body">{formatINR(l.amount)}</TableCell>
          </TableRow>
        ))}
        <TableRow className="border-t-2 bg-muted/20 font-semibold hover:bg-muted/20">
          <TableCell className="whitespace-normal text-body">{totalLabel}</TableCell>
          <TableCell className="numeric whitespace-nowrap text-right text-body">{formatINR(total)}</TableCell>
        </TableRow>
      </TableBody>
    </Table>
  );
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        <span className="text-meta text-muted-foreground">Balance Sheet is cumulative to date.</span>
      </div>
        ...
        <TabsContent value="pl" className="space-y-4">
          ...
          <Card className="border-0 shadow-card">
            ...
            <CardContent className="flex flex-wrap items-center justify-between gap-x-4 gap-y-1 py-4">
              <span className="text-meta font-semibold uppercase tracking-[0.1em] text-muted-foreground">Net {pl.netProfit >= 0 ? "profit" : "loss"} · <span className="numeric tracking-normal">FY {fy}</span></span>
              <span className={`numeric text-h2 font-bold ${pl.netProfit >= 0 ? "text-success" : "text-destructive"}`}>{formatINR(pl.netProfit)}</span>
            </CardContent>
          </Card>
        </TabsContent>
          ...
          <Card className="border-0 shadow-card">
            <CardContent className="px-0">
              ...
              <Table className="[&_td]:px-2.5 [&_th]:px-2.5 sm:[&_td]:px-4 sm:[&_th]:px-4">
                <TableHeader>
                  <TableRow className="bg-muted/30 hover:bg-muted/30 [&>th]:h-9 [&>th]:text-meta [&>th]:font-medium [&>th]:uppercase [&>th]:tracking-[0.05em] [&>th]:text-muted-foreground"><TableHead>Account</TableHead><TableHead className="text-right sm:w-40">Debit</TableHead><TableHead className="text-right sm:w-40">Credit</TableHead></TableRow>
                </TableHeader>
                <TableBody>
                  {tb.rows.length === 0 ? (
                    ...
                    <TableRow key={r.code}>
                      <TableCell className="whitespace-normal text-body"><span className="numeric text-muted-foreground">{r.code}</span> {r.name}</TableCell>
                  ...
                  <TableRow className="border-t-2 bg-muted/20 font-semibold hover:bg-muted/20">
                    <TableCell className="whitespace-normal text-body">Total {tb.balanced ? "(balanced)" : "(out of balance)"}</TableCell>
                    <TableCell className="numeric whitespace-nowrap text-right text-body">{formatINR(tb.totalDebit)}</TableCell>
                    <TableCell className="numeric whitespace-nowrap text-right text-body">{formatINR(tb.totalCredit)}</TableCell>
                  </TableRow>
                </TableBody>
              </Table>
            </CardContent>
