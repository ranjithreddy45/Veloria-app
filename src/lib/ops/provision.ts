...
export async function provisionEventOperations(
        ...
        (beoNumber) =>
          prisma.beo.create({
            data: {
              ...
              menuNotes: d?.menuNotes, floorPlanNotes: d?.floorPlanNotes, avNotes: d?.avNotes,
  ...
  // 3) Kitchen plan (one per booking) from kitchenSeed.
  try {
    ...
    } else if (template?.kitchenSeed) {
      const items = (template.kitchenSeed as unknown as KitchenSeedItem[]) ?? [];
      const est = round2(items.reduce((s, it) => s + (Number(it.quantity) || 0) * (Number(it.estUnitCost) || 0), 0));
